...
# define the grid to sample the domain
...
def gen_circular_distribution(n=500, scale=1, scale_ellipse=1):
    ...
    X = np.concatenate([np.concatenate([x1a.reshape([a+c, 1]),
                                        x1b.reshape([b, 1])]),
                        np.concatenate([x2a.reshape([a+c, 1]),
                                        x2b.reshape([b, 1])])], axis=1)
...
def gen_simple_circular_distribution(n=100, scale=1):
    ...
    X = np.concatenate([np.concatenate([x1a.reshape([a, 1]),
                                        x1b.reshape([b, 1])]),
                        np.concatenate([x2a.reshape([a, 1]),
                                        x2b.reshape([b, 1])])], axis=1)
